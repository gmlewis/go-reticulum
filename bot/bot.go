...
// Package bot is the offline survival engine shared by every Reticulum field
// appliance in this repository.
//
// It began as the headless RRC chat bot gorrcbot and was promoted to a library
// so that more than one executable can run the same intelligence in-process,
// with no radio link and no chat room behind it:
//
//   - Sensors: a pure-Go NMEA-0183 GNSS reader (gps.go) and electronic-compass
//     reader (compass.go), each of which can also hold a static fix so an
//     appliance runs with no hardware attached. The compass corrects a magnetic
//     reading to true north with the World Magnetic Model (declination.go).
//   - Geodesy: haversine distance and bearing, Maidenhead grids, Open Location
//     Codes / Plus Codes (geo.go, olc.go), and the operational position card and
//     its JSON form (whereami.go).
//   - Field tools: twenty-odd offline command handlers — wilderness first aid,
//     repeater and cell-tower steering, solar and lunar ephemeris, tides, buoys,
//     rivers, METAR, space weather, Morse, unit conversion, and the rest — each
//     one a pure function of its arguments, the clock, and the live sensors.
//   - The command registry (commands.go) that binds those handlers to names, and
//     the Engine facade (engine.go) that evaluates a command line entirely
//     in-process. Engine.Eval is the zero-hop entry point: the same answer the
//     radio would give, computed without sending a single packet.
//   - The captive web portal (portal.go, portal-page.go): an HTTP dashboard and
//     JSON API a smartphone can read after joining the appliance's network,
//     with the captive-probe routes every operating system already probes.
//
// The engine is a client, not a hub. It dials each configured RRC hub, joins the
// configured rooms, keeps them joined across link flaps, and answers only when
// it is addressed by name; Run wires that up, and cmd/gorrcbot is the thin CLI
// wrapper around it.
//
// Nothing in this package depends on anything outside the Go standard library
// and the sibling rns, rrc, and utils packages in this module.
//
// This file holds the engine itself: it dials every configured hub, keeps the
...
// programName is the prefix every operator-facing log line carries. It is
// "gorrcbot" until a caller sets it, so the bot's own history and its captured
// transcripts are unchanged by the promotion to a library.
var programName = "gorrcbot"

// SetProgramName changes the prefix on operator-facing log lines, so an
// appliance built on this package reports its own name instead of the name of
// the chat bot it grew out of. An empty or blank name is ignored. It is meant
// to be called once, before any goroutine that logs is started.
