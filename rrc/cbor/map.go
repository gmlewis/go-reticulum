...
// ToStringMap returns the map's contents as a map[string]any.
// Non-string keys are converted using fmt.Sprint.
// If m is nil, it returns nil.
func (m *Map) ToStringMap() map[string]any {
	if m == nil {
		return nil
	}
	...
	for _, p := range m.pairs {
		if s, ok := p.Key.(string); ok {
			...
			out[fmt.Sprint(p.Key)] = p.Val
		}
	}
	return out
}

// ToMap returns the map's contents as a map[any]any.
// If m is nil, it returns nil.
func (m *Map) ToMap() map[any]any {
	if m == nil {
		return nil
	}
	out := make(map[any]any, len(m.pairs))
	for _, p := range m.pairs {
		out[p.Key] = p.Val
	}
	return out
