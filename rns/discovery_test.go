...
func TestListDiscoveredInterfaces_BinaryMapKeysLogPythonKeyErrorAndRemain(t *testing.T) {
	...
	r := &Reticulum{configDir: tmpDir, logger: logger}
	discovery := NewInterfaceDiscovery(r)
	...
	if len(discovered) != 0 {
		t.Fatalf("expected 0 discovered interfaces, got %v", len(discovered))
	}
	if _, err := os.Stat(filepath.Join(storagePath, "bytes-map-key.data")); err != nil {
		t.Fatalf("expected bytes-map-key discovery file to remain on disk: %v", err)
	}

	logOutput := logs.String()
	if !strings.Contains(logOutput, "error while loading discovered interface data: 'last_heard'") {
		t.Fatalf("expected Python KeyError log, got %q", logOutput)
	}
	if !strings.Contains(logOutput, "bytes-map-key.data") || !strings.Contains(logOutput, "may be corrupt") {
		t.Fatalf("expected corrupt-file path warning in logs, got %q", logOutput)
	}
}
...
func TestListDiscoveredInterfaces_TrailingGarbageLoadsLikePython(t *testing.T) {
	...
	if discovered[0].Name != "TrailingGarbage" {
		t.Fatalf("unexpected surviving interface %q", discovered[0].Name)
	}
}
...
func TestListDiscoveredInterfaces_InvalidMsgpackLogsPythonErrorsAndRemains(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "reserved code", raw: []byte{0xc1}, wantLog: "error while loading discovered interface data: encountered reserved code: 0xc1"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(storagePath, "invalid.data"), tc.raw, 0o644); err != nil {
				t.Fatalf("failed to write invalid discovery file: %v", err)
			}
			...
			if _, err := os.Stat(filepath.Join(storagePath, "invalid.data")); err != nil {
				t.Fatalf("expected invalid discovery file to remain on disk: %v", err)
			}

			logOutput := logs.String()
			if !strings.Contains(logOutput, tc.wantLog) {
				t.Fatalf("expected log containing %q, got %q", tc.wantLog, logOutput)
			}
			if !strings.Contains(logOutput, "invalid.data") || !strings.Contains(logOutput, "may be corrupt") {
				t.Fatalf("expected corrupt-file path warning in logs, got %q", logOutput)
			}
		})
	}
...
func TestListDiscoveredInterfaces_BytesValueSortsLikePython(t *testing.T) {
	...
	entries := []struct {
		...
		{filename: "bytes-b.data", name: "BytesB", lastHeard: now - 120, value: []byte("b")},
	...
	gotNames := []string{discovered[0].Name, discovered[1].Name}
	wantNames := []string{"BytesB", "BytesA"}
	if !reflect.DeepEqual(gotNames, wantNames) {
		t.Fatalf("names = %v, want %v", gotNames, wantNames)
	}
}
...
func TestListDiscoveredInterfaces_ListValueSortsLikePython(t *testing.T) {
	...
	entries := []struct {
		...
		{filename: "list-12.data", name: "List12", lastHeard: now - 120, value: []any{1, 2}},
	...
	gotNames := []string{discovered[0].Name, discovered[1].Name}
	wantNames := []string{"List12", "List11"}
	if !reflect.DeepEqual(gotNames, wantNames) {
		t.Fatalf("names = %v, want %v", gotNames, wantNames)
	}
}
...
func TestListDiscoveredInterfaces_MixedBytesAndStringValueReturnsPythonError(t *testing.T) {
	...
	entries := []struct {
		...
		{filename: "bytes-a.data", name: "BytesA", lastHeard: now - 120, value: []byte("a")},
...
func TestListDiscoveredInterfaces_DictValuePairReturnsPythonError(t *testing.T) {
	...
	entries := []struct {
		...
		{filename: "dict-a.data", name: "DictA", lastHeard: now - 120, value: map[string]any{"a": 1}},
	...
	if _, err := discovery.ListDiscoveredInterfaces(false, false); err == nil {
		t.Fatal("ListDiscoveredInterfaces() error = nil, want error for dict value comparison")
	} else if got, want := err.Error(), `"'<' not supported between instances of 'dict' and 'dict'"`; got != want {
		t.Fatalf("ListDiscoveredInterfaces() error = %q, want %q", got, want)
	}
}
...
func TestListDiscoveredInterfaces_InvalidNetworkIDTypeLogsAndRemains(t *testing.T) {
	...
	logOutput := logs.String()
	if !strings.Contains(logOutput, "error while loading discovered interface data: fromhex() argument must be str, not int") {
		t.Fatalf("expected Python fromhex() type error log, got %q", logOutput)
	}
...
func TestListDiscoveredInterfaces_BytesNetworkIDLogsPythonTypeErrorAndRemains(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(storagePath, "bytes-network-id.data")); err != nil {
		t.Fatalf("expected bytes network_id discovery file to remain on disk: %v", err)
	}

	logOutput := logs.String()
	if !strings.Contains(logOutput, "error while loading discovered interface data: fromhex() argument must be str, not bytes") {
		t.Fatalf("expected Python fromhex() bytes type error log, got %q", logOutput)
	}
	if !strings.Contains(logOutput, "bytes-network-id.data") || !strings.Contains(logOutput, "may be corrupt") {
		t.Fatalf("expected corrupt-file path warning in logs, got %q", logOutput)
	}
}
...
func TestPersistDiscoveredInterface_NewEntryMissingReceivedLeavesEmptyFile(t *testing.T) {
	...
	} else if got, want := err.Error(), "'received'"; got != want {
		t.Fatalf("persistDiscoveredInterface() error = %q, want %q", got, want)
	}
...
func TestPersistDiscoveredInterface_NewEntryAllowsBoolReceived(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "discovered")); got != tc.want {
				t.Fatalf("discovered = %v, want %v", got, tc.want)
			}
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "last_heard")); got != tc.want {
				t.Fatalf("last_heard = %v, want %v", got, tc.want)
			}
			if got := asInt(lookupAnyValue(m, "heard_count")); got != 0 {
				t.Fatalf("heard_count = %v, want 0", got)
			}
...
func TestPersistDiscoveredInterface_NewEntryAllowsCompositeReceived(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		{name: "bytes", got: []byte("x"), want: "b'x'"},
		{name: "list", got: []any{1}, want: "[1]"},
...
func TestPersistDiscoveredInterface_ExistingEntryMissingReceivedMutatesDiscoveredBeforeTruncatingFile(t *testing.T) {
	t.Parallel()

	for _, tc := range []struct {
		name              string
		seedDiscovered    any
		incomingDiscovery any
		wantDiscovered    any
	}{
		{name: "cached discovered", seedDiscovered: 5.0, wantDiscovered: 5.0},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			path := filepath.Join(storagePath, "aabbcc.data")
			if err := os.WriteFile(path, mustMsgpackPack(map[string]any{
				"discovered":  tc.seedDiscovered,
				"heard_count": 2,
			}), 0o644); err != nil {
				t.Fatalf("failed to seed persisted discovery file: %v", err)
			}
			...
			if tc.incomingDiscovery != nil {
				info["discovered"] = tc.incomingDiscovery
			}
			...
			if got := pythonDiscoveryReprString(info["discovered"]); got != pythonDiscoveryReprString(tc.wantDiscovered) {
				t.Fatalf("info[\"discovered\"] = %v, want %v", got, pythonDiscoveryReprString(tc.wantDiscovered))
			}
			if _, ok := info["last_heard"]; ok {
				t.Fatalf("info[\"last_heard\"] unexpectedly set: %v", info["last_heard"])
			}
...
func TestPersistDiscoveredInterface_ExistingEntryAllowsBoolReceived(t *testing.T) {
	t.Parallel()
...
func TestPersistDiscoveredInterface_ExistingEntryAllowsCompositeReceived(t *testing.T) {
	t.Parallel()
...
func TestPersistDiscoveredInterface_ExistingEntryStringHeardCountTruncatesFile(t *testing.T) {
	...
	} else if got, want := err.Error(), `can only concatenate str (not "int") to str`; got != want {
		t.Fatalf("persistDiscoveredInterface() error = %q, want %q", got, want)
	}
	if got := asFloat64(info["discovered"]); got != 5.0 {
		t.Fatalf("info[\"discovered\"] = %v, want 5.0", got)
	}
	if got := asFloat64(info["last_heard"]); got != 1.0 {
		t.Fatalf("info[\"last_heard\"] = %v, want 1.0", got)
	}
	if _, ok := info["heard_count"]; ok {
		t.Fatalf("info[\"heard_count\"] unexpectedly set: %v", info["heard_count"])
	}
...
func TestInterfaceDiscoveryAutoconnectMissingNetworkIDLogsPythonKeyError(t *testing.T) {
	...
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces when network_id is missing, got %v", got)
	}

	want := "Error while auto-connecting discovered interface: 'network_id'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectMissingTypeLogsPythonKeyError(t *testing.T) {
	...
	info := DiscoveredInterface{
		...
		hasConfigEntry: true,
		hasNetworkID:   true,
		endpoint:       newDiscoveryEndpoint("127.0.0.1", true, 4242, true),
	}
	...
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces when type is missing, got %v", got)
	}

	want := "Error while auto-connecting discovered interface: 'type'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectMissingNameLogsPythonKeyError(t *testing.T) {
	...
	if calls != 0 {
		t.Fatalf("backboneFactory calls = %v, want 0 when name is missing", calls)
	}
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces when name is missing, got %v", got)
	}

	want := "Error while auto-connecting discovered interface: 'name'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectListPortLogsPythonTypeError(t *testing.T) {
	...
	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "List Port Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[list-port]]",
		"reachable_on": "127.0.0.1",
		"port":         []any{1},
		"network_id":   "01020304",
	})
	...
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces for list-typed port, got %v", got)
	}

	want := "Error while auto-connecting discovered interface: int() argument must be a string, a bytes-like object or a real number, not 'list'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectBytesReachableOnLogsPythonTypeError(t *testing.T) {
	...
	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Bytes Reachable Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[bytes-reachable]]",
		"reachable_on": []byte("127.0.0.1"),
		"port":         4242,
		"network_id":   "01020304",
	})
	...
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces for bytes reachable_on, got %v", got)
	}

	want := "Error while auto-connecting discovered interface: a bytes-like object is required, not 'str'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectCompositeReachableOnUsesPythonDisplayHost(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "list", reachableOn: []any{1}, wantHost: "[1]"},
		{name: "dict", reachableOn: map[string]any{"a": 1}, wantHost: "{'a': 1}"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			info, ok := mapToDiscoveredInterface(map[string]any{
				"name":         "Composite Reachable Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[composite-reachable]]",
				"reachable_on": tc.reachableOn,
				"port":         4242,
				"network_id":   "01020304",
			})
			if !ok {
			...
			meta, ok := iface.(interface{ TargetHost() string })
			if !ok {
				t.Fatalf("auto-connected interface %T does not expose TargetHost()", iface)
			}
			if got := meta.TargetHost(); got != tc.wantHost {
				t.Fatalf("TargetHost() = %q, want %q", got, tc.wantHost)
			}

			for _, msg := range logs {
				if strings.Contains(msg, "Error while auto-connecting discovered interface:") {
					t.Fatalf("unexpected discovery error log: %v", logs)
				}
			}
		})
	}
...
func TestInterfaceDiscoveryAutoconnectScalarReachableOnLogsPythonTypeError(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "int", reachableOn: 1, wantType: "int"},
		{name: "bool", reachableOn: true, wantType: "bool"},
		{name: "float", reachableOn: 1.5, wantType: "float"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			info, ok := mapToDiscoveredInterface(map[string]any{
				"name":         "Scalar Reachable Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[scalar-reachable]]",
				"reachable_on": tc.reachableOn,
				"port":         4242,
				"network_id":   "01020304",
			})
			...
			if got := len(ts.GetInterfaces()); got != 0 {
				t.Fatalf("expected no auto-connected interfaces for scalar reachable_on, got %v", got)
			}

			want := "Error while auto-connecting discovered interface: argument of type '" + tc.wantType + "' is not iterable"
			for _, msg := range logs {
				if strings.Contains(msg, want) {
					return
				}
...
func TestInterfaceDiscoveryAutoconnectNilReachableOnRegistersDormantInterface(t *testing.T) {
	t.Parallel()
	...
	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Nil Reachable Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[nil-reachable]]",
		"reachable_on": nil,
		"port":         4242,
		"network_id":   "01020304",
	})
	...
	if got := len(ts.GetInterfaces()); got != 1 {
		t.Fatalf("expected 1 dormant auto-connected interface, got %v", got)
	}
	...
	if !ok {
		t.Fatalf("auto-connected interface %T does not expose expected metadata", iface)
	}
	if got := meta.AutoconnectSource(); got != "01020304" {
		t.Fatalf("AutoconnectSource() = %q, want %q", got, "01020304")
	}
	if got := meta.TargetHost(); got != "" {
		t.Fatalf("TargetHost() = %q, want empty string for nil reachable_on", got)
	}
	if got := meta.TargetPort(); got != 0 {
		t.Fatalf("TargetPort() = %v, want 0 for nil reachable_on", got)
	}
	if meta.Status() {
		t.Fatal("expected dormant auto-connected interface to remain offline")
	}
...
func TestInterfaceDiscoveryAutoconnectRecoversRegisterInterfacePanic(t *testing.T) {
	t.Parallel()
	...
	monitorCalled := false
	discovery.registerAutoconnect = func(interfaces.Interface) {
		panic("add boom")
	}
	discovery.monitorAutoconnect = func(interfaces.Interface) {
		monitorCalled = true
	}

	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Register Panic Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[register-panic]]",
		"reachable_on": nil,
		"port":         4242,
		"network_id":   "01020304",
	})
	...
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected register panic to leave transport unchanged, got %v interfaces", got)
	}
	if monitorCalled {
		t.Fatal("expected monitor hook to be skipped after register panic")
	}

	want := "Error while auto-connecting discovered interface: add boom"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectRecoversMonitorInterfacePanic(t *testing.T) {
	t.Parallel()
	...
	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Monitor Panic Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[monitor-panic]]",
		"reachable_on": "127.0.0.1",
		"port":         4242,
		"network_id":   "01020304",
	})
	...
	if got := len(ts.GetInterfaces()); got != 1 {
		t.Fatalf("expected registered interface to remain after monitor panic, got %v interfaces", got)
	}

	want := "Error while auto-connecting discovered interface: monitor boom"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryAutoconnectLogsConstructorError(t *testing.T) {
	t.Parallel()
	...
	discovery.backboneFactory = func(config discoveryBackboneClientConfig, handler interfaces.InboundHandler) (interfaces.Interface, error) {
		return nil, fmt.Errorf("ctor boom")
	}

	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Ctor Boom Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[ctor-boom]]",
		"reachable_on": "127.0.0.1",
		"port":         4242,
		"network_id":   "01020304",
	})
	...
	if err := discovery.autoconnect(info); err != nil {
		t.Fatalf("autoconnect() error = %v, want nil after constructor error logging", err)
	}
	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected constructor error to leave transport unchanged, got %v interfaces", got)
	}

	want := "Error while auto-connecting discovered interface: ctor boom"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryInterfaceExistsCompositeReachableOnDoesNotMatchStringTargetHost(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{name: "list", reachableOn: []any{1}, targetHost: "[1]"},
		{name: "dict", reachableOn: map[string]any{"a": 1}, targetHost: "{'a': 1}"},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			ts.RegisterInterface(newTargetHostTestInterface("Existing Backbone", "BackboneClientInterface", tc.targetHost, 4242))
			...
			if discovery.interfaceExists(info) {
				t.Fatalf("expected composite reachable_on %T not to match existing string target host %q", tc.reachableOn, tc.targetHost)
			}
		})
	}
}

func TestInterfaceDiscoveryInterfaceExistsMatchesBytesReachableOnAutoconnectHash(t *testing.T) {
	t.Parallel()
	...
	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Bytes Host Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[bytes-host]]",
		"reachable_on": []byte("127.0.0.1"),
		"port":         4242,
	})
	if !ok {
	...
	ts.RegisterInterface(&autoconnectCountTestInterface{
		BaseInterface:   interfaces.NewBaseInterface("Existing Bytes Host", interfaces.ModeFull, 1000),
		autoconnectHash: discovery.endpointHash(info),
	})

	if !discovery.interfaceExists(info) {
		t.Fatal("expected bytes reachable_on discovery info to match Python-shaped autoconnect hash")
	}
}

func TestInterfaceDiscoveryInterfaceExistsPresentNilReachableOnDoesNotMatchExistingTargetHost(t *testing.T) {
	t.Parallel()
	...
	ts.RegisterInterface(newTargetHostTestInterface("Existing Nil Host", "BackboneClientInterface", "", 0))

	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Nil Host Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[nil-host]]",
		"reachable_on": nil,
	})
	if !ok {
		t.Fatal("mapToDiscoveredInterface() = false, want true")
	...
	if discovery.interfaceExists(info) {
		t.Fatal("expected present nil reachable_on not to match an existing string target host")
	}
}

func TestInterfaceDiscoveryInterfaceExistsNonIntegralFloatPortDoesNotMatchExistingTargetPort(t *testing.T) {
	t.Parallel()
	...
	ts.RegisterInterface(newTargetHostTestInterface("Existing Float Port", "BackboneClientInterface", "127.0.0.1", 1))

	info, ok := mapToDiscoveredInterface(map[string]any{
		"name":         "Float Port Backbone",
		"type":         "BackboneInterface",
		"config_entry": "[[float-port]]",
		"reachable_on": "127.0.0.1",
		"port":         1.5,
	})
	if !ok {
	...
	if discovery.interfaceExists(info) {
		t.Fatal("expected non-integral float port not to match an existing concrete target port")
	}
}
...
func TestInterfaceDiscoveryInterfaceExistsMatchesPythonAutoconnectHashForRawEndpointValues(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "nil host with port",
			info: map[string]any{
				"name":         "Nil Host Port Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[nil-host-port]]",
				"reachable_on": nil,
				"port":         4242,
			},
		},
		{
			name: "nil host without port",
			info: map[string]any{
				"name":         "Nil Host Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[nil-host]]",
		...
		},
		{
			name: "bytes port",
			info: map[string]any{
				"name":         "Bytes Port Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[bytes-port]]",
				"reachable_on": "127.0.0.1",
				"port":         []byte("1"),
			},
		},
		{
			name: "non-integral float port",
			info: map[string]any{
				"name":         "Float Port Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[float-port]]",
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			info, ok := mapToDiscoveredInterface(tc.info)
			if !ok {
				t.Fatal("mapToDiscoveredInterface() = false, want true")
			}

			ts.RegisterInterface(&autoconnectCountTestInterface{
				BaseInterface:   interfaces.NewBaseInterface("Existing Raw Endpoint", interfaces.ModeFull, 1000),
				autoconnectHash: discovery.endpointHash(info),
			})

			if !discovery.interfaceExists(info) {
				t.Fatalf("expected discovery info %+v to match Python-shaped autoconnect hash", tc.info)
			}
		})
	}
}
...
func TestInterfaceDiscoveryInterfaceExistsDirectPortComparisonMatchesPythonEquality(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "bool true", port: true, wantHit: true},
		{name: "integral float", port: 1.0, wantHit: true},
		{name: "bytes", port: []byte("1"), wantHit: false},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			info, ok := mapToDiscoveredInterface(map[string]any{
				"name":         "Direct Match Backbone",
				"type":         "BackboneInterface",
				"config_entry": "[[direct-match]]",
				"reachable_on": "127.0.0.1",
				"port":         tc.port,
				"network_id":   "01020304",
			})
			...
			if got := discovery.interfaceExists(info); got != tc.wantHit {
				t.Fatalf("interfaceExists(port=%T(%v)) = %v, want %v", tc.port, tc.port, got, tc.wantHit)
			}
		})
	}
}
...
func TestInterfaceDiscoveryStartCallbackBytesReachableOnLogsPythonTypeErrorAndSkipsAutoconnect(t *testing.T) {
	...
	case acceptedConn := <-accepted:
		...
		t.Fatal("unexpected callback autoconnect with bytes reachable_on")
	case <-time.After(300 * time.Millisecond):
	}

	if got := len(ts.GetInterfaces()); got != 1 {
		t.Fatalf("expected only the existing interface after bytes reachable_on failure, got %v", got)
	}
...
func TestInterfaceDiscoveryStartInvokesDiscoveryCallbackWhenDuplicateAutoconnectSkipped(t *testing.T) {
	...
	logger := NewLogger()
	ts := NewTransportSystem(logger)
	ts.RegisterInterface(newTargetHostTestInterface("Existing Backbone", "BackboneClientInterface", "127.0.0.1", 4242))
	...
	case result = <-callbackCh:
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for duplicate-autoconnect discovery callback")
	}

	if result.err != nil {
		t.Fatalf("callback ListDiscoveredInterfaces() error = %v", result.err)
	}
	if result.name != "Existing Backbone" {
		t.Fatalf("callback name = %q, want %q", result.name, "Existing Backbone")
	}
	if result.discovered != receivedAt {
		t.Fatalf("callback discovered = %v, want %v", result.discovered, receivedAt)
	}
	if result.lastHeard != receivedAt {
		t.Fatalf("callback last_heard = %v, want %v", result.lastHeard, receivedAt)
	}
	if result.heardCount != 0 {
		t.Fatalf("callback heard_count = %v, want 0", result.heardCount)
	}
	if result.interfacesCount != 1 {
		t.Fatalf("callback interfaces count = %v, want 1", result.interfacesCount)
	}
...
func TestInterfaceDiscoveryStartCallbackReceivesRediscoveryMetadataForBoolReceived(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Bool Received Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"hops":           1,
			...
			if got := pythonDiscoveryReprString(callbackInfo["last_heard"]); got != tc.want {
				t.Fatalf("callback last_heard = %v, want %v", got, tc.want)
			}
			if got := asInt(callbackInfo["heard_count"]); got != 3 {
				t.Fatalf("callback heard_count = %v, want 3", got)
			}
			...
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "last_heard")); got != tc.want {
				t.Fatalf("persisted last_heard = %v, want %v", got, tc.want)
			}
			if got := asInt(lookupAnyValue(m, "heard_count")); got != 3 {
				t.Fatalf("persisted heard_count = %v, want 3", got)
			}
...
func TestInterfaceDiscoveryStartCallbackReceivesRediscoveryMetadataForCompositeReceived(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Composite Received Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"hops":           1,
...
func TestInterfaceDiscoveryStartCallbackReceivesRediscoveryMetadataForLiveDiscoveredReplacement(t *testing.T) {
	t.Parallel()

	for _, tc := range []struct {
		name                 string
		liveDiscovered       any
		wantDiscoveredFloat  float64
		wantDiscoveredString string
		wantNilDiscovered    bool
	}{
		{
			name:              "nil replacement",
			liveDiscovered:    nil,
			wantNilDiscovered: true,
		},
		{
			name:                 "string replacement",
			liveDiscovered:       "live-seeded",
			wantDiscoveredString: "live-seeded",
		},
		{
			name:                "float replacement",
			liveDiscovered:      4.5,
			wantDiscoveredFloat: 4.5,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			receivedAt := 1234.0
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Live Discovered Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"config_entry":   "[[rediscovery-live-discovered]]",
				"hops":           1,
				"received":       receivedAt,
				"reachable_on":   "127.0.0.1",
				"port":           listener.Addr().(*net.TCPAddr).Port,
				"network_id":     "01020304",
				"discovered":     tc.liveDiscovered,
			})
			...
			if tc.wantNilDiscovered {
				if got, ok := callbackInfo["discovered"]; !ok || got != nil {
					t.Fatalf("callback discovered = %#v, present=%v, want nil present", got, ok)
				}
			} else if tc.wantDiscoveredString != "" {
				if got := asString(callbackInfo["discovered"]); got != tc.wantDiscoveredString {
					t.Fatalf("callback discovered = %q, want %q", got, tc.wantDiscoveredString)
				}
			} else if got := asFloat64(callbackInfo["discovered"]); got != tc.wantDiscoveredFloat {
				t.Fatalf("callback discovered = %v, want %v", got, tc.wantDiscoveredFloat)
			}
			if got := asFloat64(callbackInfo["last_heard"]); got != receivedAt {
				t.Fatalf("callback last_heard = %v, want %v", got, receivedAt)
			}
			...
			if tc.wantNilDiscovered {
				if got, ok := lookupAny(m, "discovered"); !ok || got != nil {
					t.Fatalf("persisted discovered = %#v, present=%v, want nil present", got, ok)
				}
			} else if tc.wantDiscoveredString != "" {
				if got := asString(lookupAnyValue(m, "discovered")); got != tc.wantDiscoveredString {
					t.Fatalf("persisted discovered = %q, want %q", got, tc.wantDiscoveredString)
				}
			} else if got := asFloat64(lookupAnyValue(m, "discovered")); got != tc.wantDiscoveredFloat {
				t.Fatalf("persisted discovered = %v, want %v", got, tc.wantDiscoveredFloat)
			}
			if got := asFloat64(lookupAnyValue(m, "last_heard")); got != receivedAt {
				t.Fatalf("persisted last_heard = %v, want %v", got, receivedAt)
			}
...
func TestInterfaceDiscoveryStartCallbackReceivesRediscoveryMetadataForNilCachedDiscovered(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			receivedAt := 1234.0
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Nil Cached Discovered Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"config_entry":   "[[rediscovery-nil-cached-discovered]]",
				"hops":           1,
				"received":       receivedAt,
				"reachable_on":   "127.0.0.1",
				"port":           listener.Addr().(*net.TCPAddr).Port,
...
func TestInterfaceDiscoveryStartCallbackPreservesExoticCachedDiscoveredValues(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		{name: "string", discovered: "cached-string", wantRepr: "'cached-string'"},
		{name: "bool", discovered: true, wantRepr: "True"},
		{name: "bytes", discovered: []byte("x"), wantRepr: "b'x'"},
		{name: "list", discovered: []any{1}, wantRepr: "[1]"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			receivedAt := 1234.0
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Exotic Cached Discovered Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"hops":           1,
			...
			if got := pythonDiscoveryReprString(callbackInfo["discovered"]); got != tc.wantRepr {
				t.Fatalf("callback discovered = %v, want %v", got, tc.wantRepr)
			}
			if got := asFloat64(callbackInfo["last_heard"]); got != receivedAt {
				t.Fatalf("callback last_heard = %v, want %v", got, receivedAt)
			}
			...
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "discovered")); got != tc.wantRepr {
				t.Fatalf("persisted discovered = %v, want %v", got, tc.wantRepr)
			}
			if got := asFloat64(lookupAnyValue(m, "last_heard")); got != receivedAt {
				t.Fatalf("persisted last_heard = %v, want %v", got, receivedAt)
			}
...
func TestInterfaceDiscoveryStartCallbackPreservesFalsishRediscoveryValues(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range []struct {
		...
		wantHeardCount string
	}{
		{name: "false discovered", seedDiscovered: false, seedHeardCount: 2, wantDiscovered: "False", wantHeardCount: "3"},
		{name: "false heard count", seedDiscovered: 0, seedHeardCount: false, wantDiscovered: "0", wantHeardCount: "1"},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			receivedAt := 9.0
			discovery.handler.callback(map[string]any{
				"name":           "Rediscovery Falsish Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc},
				"hops":           1,
			...
			if got := pythonDiscoveryReprString(callbackInfo["discovered"]); got != tc.wantDiscovered {
				t.Fatalf("callback discovered = %v, want %v", got, tc.wantDiscovered)
			}
			if got := asFloat64(callbackInfo["last_heard"]); got != receivedAt {
				t.Fatalf("callback last_heard = %v, want %v", got, receivedAt)
			}
			if got := pythonDiscoveryReprString(callbackInfo["heard_count"]); got != tc.wantHeardCount {
				t.Fatalf("callback heard_count = %v, want %v", got, tc.wantHeardCount)
			}
			...
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "discovered")); got != tc.wantDiscovered {
				t.Fatalf("persisted discovered = %v, want %v", got, tc.wantDiscovered)
			}
			if got := asFloat64(lookupAnyValue(m, "last_heard")); got != receivedAt {
				t.Fatalf("persisted last_heard = %v, want %v", got, receivedAt)
			}
			if got := pythonDiscoveryReprString(lookupAnyValue(m, "heard_count")); got != tc.wantHeardCount {
				t.Fatalf("persisted heard_count = %v, want %v", got, tc.wantHeardCount)
			}
		})
	}
}
...
func TestInterfaceDiscoveryStartCallbackIterableDiscoveryHashLogsPythonFormatErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "slice-string-element",
			hash: []any{"a"},
			want: "Error processing discovered interface data: Unknown format code 'x' for object of type 'str'",
		},
		{
			name: "slice-float-element",
			hash: []any{1.5},
			want: "Error processing discovered interface data: Unknown format code 'x' for object of type 'float'",
		},
		{
			name: "slice-bytes-element",
			hash: []any{[]byte{0xaa}},
			want: "Error processing discovered interface data: unsupported format string passed to bytes.__format__",
		},
		{
			name: "map-string-key",
			hash: map[string]any{"a": "ignored"},
			want: "Error processing discovered interface data: Unknown format code 'x' for object of type 'str'",
		},
		{
			name: "map-float-key",
			hash: map[float64]any{1.5: "ignored"},
			want: "Error processing discovered interface data: Unknown format code 'x' for object of type 'float'",
		},
		{
			name: "slice-nested-list",
			hash: []any{[]any{1}},
			want: "Error processing discovered interface data: unsupported format string passed to list.__format__",
		},
		{
			name: "slice-nested-dict",
			hash: []any{map[string]any{"a": 1}},
			want: "Error processing discovered interface data: unsupported format string passed to dict.__format__",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			info := map[string]any{
				"name":           "Iterable Error Hash TCP",
				"type":           "TCPServerInterface",
				"discovery_hash": tc.hash,
				"received":       1234.0,
				"hops":           1,
				"value":          12,
			}
			...
			if callbackCalled {
				t.Fatal("expected external discovery callback not to run for malformed iterable discovery hash")
			}
			if _, ok := info["discovered"]; ok {
				t.Fatalf("info[\"discovered\"] unexpectedly set: %v", info["discovered"])
			}
			...
			if len(matches) != 0 {
				t.Fatalf("expected malformed iterable discovery hash not to be persisted, got %v", matches)
			}
			for _, msg := range logs {
				if strings.Contains(msg, tc.want) {
					return
...
func TestInterfaceDiscoveryStartLogsPythonPersistTypeErrorForListCache(t *testing.T) {
	...
	if !bytes.Equal(data, original) {
		t.Fatalf("cached discovery file changed on list-cache failure: got %x want %x", data, original)
	}

	want := "Error while persisting discovered interface data: list indices must be integers or slices, not str"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryStartLogsPythonPersistTypeErrorForStringCache(t *testing.T) {
	...
	storagePath := filepath.Join(tmpDir, "discovery", "interfaces")
	filePath := filepath.Join(storagePath, "aabbccdd.data")
	original := mustMsgpackPack("abc")
	if err := os.WriteFile(filePath, original, 0o644); err != nil {
		t.Fatalf("failed to seed cached discovery file: %v", err)
	}

	discovery.handler.callback(map[string]any{
		"name":           "Persist String Cache Backbone",
		"value":          7,
		"type":           "BackboneInterface",
		"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
		"hops":           2,
	...
	if !bytes.Equal(data, original) {
		t.Fatalf("cached discovery file changed on string-cache failure: got %x want %x", data, original)
	}

	want := "Error while persisting discovered interface data: string indices must be integers, not 'str'"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryStartLogsPythonPersistExtraDataForInvalidMsgpackCache(t *testing.T) {
	...
	storagePath := filepath.Join(tmpDir, "discovery", "interfaces")
	filePath := filepath.Join(storagePath, "aabbccdd.data")
	original := []byte("not-msgpack")
	if err := os.WriteFile(filePath, original, 0o644); err != nil {
		t.Fatalf("failed to seed cached discovery file: %v", err)
	}

	discovery.handler.callback(map[string]any{
		"name":           "Persist Invalid Msgpack Cache Backbone",
		"value":          7,
		"type":           "BackboneInterface",
		"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
		"hops":           2,
...
func TestInterfaceDiscoveryStartLogsPythonPersistFormatErrorForInvalidTypeCodeCache(t *testing.T) {
	...
	discovery.handler.callback(map[string]any{
		"name":           "Persist Invalid Type Code Cache Backbone",
		"value":          7,
		"type":           "BackboneInterface",
		"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
		"hops":           2,
...
func TestInterfaceDiscoveryStartLogsPythonPersistValueErrorForIncompleteMsgpackCache(t *testing.T) {
	...
	storagePath := filepath.Join(tmpDir, "discovery", "interfaces")
	filePath := filepath.Join(storagePath, "aabbccdd.data")
	original := []byte{0x81, 0xa1, 'a'}
	if err := os.WriteFile(filePath, original, 0o644); err != nil {
		t.Fatalf("failed to seed cached discovery file: %v", err)
	}

	discovery.handler.callback(map[string]any{
		"name":           "Persist Incomplete Msgpack Cache Backbone",
		"value":          7,
		"type":           "BackboneInterface",
		"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
		"hops":           2,
...
func TestInterfaceDiscoveryStartLogsPythonPersistTypeErrorForStringHeardCount(t *testing.T) {
	...
	info := map[string]any{
		"name":           "Persist TypeError Backbone",
		"value":          7,
		"type":           "BackboneInterface",
		"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
		"hops":           2,
	...
	want := `Error while persisting discovered interface data: can only concatenate str (not "int") to str`
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryStartLogsPythonPersistKeyErrorForMissingReceived(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "new entry"},
		{name: "existing entry", seedExisting: true, seedDiscovered: 5.0, wantDiscovered: 5.0},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			info := map[string]any{
				"name":           "Persist Missing Received Backbone",
				"value":          7,
				"type":           "BackboneInterface",
				"discovery_hash": []byte{0xaa, 0xbb, 0xcc, 0xdd},
				"hops":           2,
			...
			} else if got := pythonDiscoveryReprString(info["discovered"]); got != pythonDiscoveryReprString(tc.wantDiscovered) {
				t.Fatalf("info[\"discovered\"] = %v, want %v", got, pythonDiscoveryReprString(tc.wantDiscovered))
			}
			if _, ok := info["last_heard"]; ok {
				t.Fatalf("info[\"last_heard\"] unexpectedly set: %v", info["last_heard"])
			...
			if err != nil {
				t.Fatalf("expected cached discovery file after persist failure: %v", err)
			}
			if fi.Size() != 0 {
				t.Fatalf("cached discovery file size = %v, want 0", fi.Size())
			}

			want := "Error while persisting discovered interface data: 'received'"
			for _, msg := range logs {
				if strings.Contains(msg, want) {
					return
				}
...
func TestInterfaceDiscoveryMonitorLogsDisconnectReconnectAndDetach(t *testing.T) {
	t.Parallel()
	...
	for _, want := range []string{
		"Auto-discovered interface monitored disconnected",
		"Auto-discovered interface monitored reconnected",
		"Auto-discovered interface monitored has been down for",
		"detaching",
	} {
		found := false
		for _, msg := range logs {
...
func TestInterfaceDiscoveryMonitorDoesNotAutoconnectBeforeInitialPass(t *testing.T) {
	...
	port := listener.Addr().(*net.TCPAddr).Port
	now := float64(time.Now().UnixNano()) / 1e9
	if err := os.WriteFile(filepath.Join(storagePath, "before-initial.data"), mustMsgpackPack(map[string]any{
		"name":         "Before Initial Candidate",
		"type":         "BackboneInterface",
		"transport":    true,
		"last_heard":   now - 30,
		"discovered":   now - 60,
		"value":        1,
		"config_entry": "[[Before Initial Candidate]]",
		"reachable_on": "127.0.0.1",
		"port":         port,
		"network_id":   "0a0b0c0d",
	}), 0o644); err != nil {
	...
	discovery.monitorAutoconnectsOnce(time.Unix(299, 0))

	if got := len(ts.GetInterfaces()); got != 0 {
		t.Fatalf("expected no auto-connected interfaces before the initial pass, got %v", got)
	}

	select {
	case conn := <-accepted:
		_ = conn.Close()
		t.Fatal("unexpected connection before initial autoconnect pass completed")
	case <-time.After(200 * time.Millisecond):
	}
}
...
func TestInterfaceDiscoveryMonitorDoesNotAutoconnectWhenFreeSlotsEqualReservedSlots(t *testing.T) {
	...
	port := listener.Addr().(*net.TCPAddr).Port
	now := float64(time.Now().UnixNano()) / 1e9
	if err := os.WriteFile(filepath.Join(storagePath, "reserved-slot.data"), mustMsgpackPack(map[string]any{
		"name":         "Reserved Slot Candidate",
		"type":         "BackboneInterface",
		"transport":    true,
		"last_heard":   now - 30,
		"discovered":   now - 60,
		"value":        1,
		"config_entry": "[[Reserved Slot Candidate]]",
		"reachable_on": "127.0.0.1",
		"port":         port,
		"network_id":   "0e0f1011",
	}), 0o644); err != nil {
	...
	if got := len(ts.GetInterfaces()); got != 3 {
		t.Fatalf("expected reserved slots to prevent new autoconnect, got %v interfaces", got)
	}

	select {
	case conn := <-accepted:
		_ = conn.Close()
		t.Fatal("unexpected connection when free slots only match reserved slots")
	case <-time.After(200 * time.Millisecond):
	}
}
...
func TestInterfaceDiscoveryMonitorLogsBootstrapTeardown(t *testing.T) {
	t.Parallel()
	...
	want := "Tearing down bootstrap-only bootstrap since target connected auto-discovered interface count has been reached"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceDiscoveryMonitorLogsBootstrapReenable(t *testing.T) {
	t.Parallel()
	...
	reenabled := 0
	owner.registerBootstrapRestarter(func() error {
		reenabled++
		return nil
	})
	...
	discovery.monitorAutoconnectsOnce(time.Unix(402, 0))

	if reenabled != 1 {
		t.Fatalf("expected one bootstrap re-enable attempt, got %v", reenabled)
	}

	want := "No auto-discovered interfaces connected, re-enabling bootstrap interfaces"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func (ts *announceCaptureTransport) Outbound(packet *Packet) error {
	...
	ts.mu.Unlock()
	if ts.outboundErr != nil {
		return ts.outboundErr
	}
	select {
...
func TestInterfaceAnnouncerLogsCouldNotGenerateForUnsupportedInterface(t *testing.T) {
	...
	iface := &announceTestInterface{
		BaseInterface: interfaces.NewBaseInterface("announce-unsupported", interfaces.ModeGateway, 1000),
		ifaceType:     "UnsupportedInterface",
	}
	iface.SetDiscoveryConfig(interfaces.DiscoveryConfig{
		SupportsDiscovery: true,
		Discoverable:      true,
		AnnounceInterval:  time.Hour,
		Name:              "Unsupported",
		ReachableOn:       "unsupported.example.net",
	})
	ts.RegisterInterface(iface)

	announcer.announceOnce(time.Unix(500, 0))

	want := "Could not generate interface discovery announce data for announce-unsupported"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
...
func TestInterfaceAnnouncerLogsAbortForInvalidReachableOn(t *testing.T) {
	t.Parallel()
	...
	iface := &announceTestInterface{
		BaseInterface: interfaces.NewBaseInterface("announce-invalid", interfaces.ModeGateway, 1000),
		ifaceType:     "BackboneInterface",
		bindIP:        "127.0.0.1",
		bindPort:      4245,
	}
	iface.SetDiscoveryConfig(interfaces.DiscoveryConfig{
		...
		Discoverable:      true,
		AnnounceInterval:  time.Hour,
		Name:              "Invalid ReachableOn",
		ReachableOn:       "bad host !",
	})
	ts.RegisterInterface(iface)
	...
	for _, want := range []string{
		"The configured reachable_on parameter",
...
func TestInterfaceAnnouncerLogsAbortForMissingNetworkIdentityEncryption(t *testing.T) {
	t.Parallel()
	...
	iface := &announceTestInterface{
		BaseInterface: interfaces.NewBaseInterface("announce-encrypt", interfaces.ModeGateway, 1000),
		ifaceType:     "BackboneInterface",
		bindIP:        "127.0.0.1",
		bindPort:      4246,
	}
	iface.SetDiscoveryConfig(interfaces.DiscoveryConfig{
		...
		ReachableOn:       "encrypt.example.net",
		Encrypt:           true,
	})
	ts.RegisterInterface(iface)
	...
	for _, want := range []string{
		"Discovery encryption requested for announce-encrypt, but no network identity configured. Aborting discovery announce.",
		"Could not generate interface discovery announce data for announce-encrypt",
	} {
		found := false
		for _, msg := range logs {
			if strings.Contains(msg, want) {
...
func TestInterfaceAnnouncerLogsSendFailureAsPrepareError(t *testing.T) {
	t.Parallel()
	...
	ts := newAnnounceCaptureTransport(logger)
	ts.outboundErr = errors.New("send boom")
	transportIdentity := mustTestNewIdentity(t, true)
	ts.identity = transportIdentity
	ts.SetEnabled(true)
	...
	iface := &announceTestInterface{
		BaseInterface: interfaces.NewBaseInterface("announce-send-fail", interfaces.ModeGateway, 1000),
		ifaceType:     "BackboneInterface",
		bindIP:        "127.0.0.1",
		bindPort:      4247,
	}
	iface.SetDiscoveryConfig(interfaces.DiscoveryConfig{
		...
		AnnounceInterval:  time.Hour,
		Name:              "Send Failure",
		ReachableOn:       "send.example.net",
	})
	ts.RegisterInterface(iface)

	announcer.announceOnce(time.Unix(503, 0))

	want := "Error while preparing interface discovery announces: send boom"
	for _, msg := range logs {
		if strings.Contains(msg, want) {
			return
		}
