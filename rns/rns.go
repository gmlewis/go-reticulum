...
func (r *Reticulum) registerBootstrapRestarter(restarter func() error) {
	if r == nil || restarter == nil {
		return
	}
	r.mu.Lock()
	defer r.mu.Unlock()
	r.bootstrapRestarters = append(r.bootstrapRestarters, restarter)
}
...
func (r *Reticulum) initInterfaces() error {
	...
	for _, sub := range interfacesSection.Subsections {
		...
		selectedMode := parseInterfaceMode(sub, ifaceType)
		discoveryConfig, selectedMode := parseDiscoveryConfig(sub, ifaceType, selectedMode)
		bootstrapOnly := false
		...
		case "TCPInterface", "TCPClientInterface", "TCPServerInterface":
			mode, _ := sub.GetProperty("mode")
			if ifaceType == "TCPClientInterface" || (ifaceType == "TCPInterface" && mode == "client") || (ifaceType == "TCPInterface" && mode == "") {
