...
// TestRelayLinkProofMarksDestinationUsed covers Phase 13 task 8 site A: when a
...
// TestPathRequestForLocalClientDestinationMarksUsed covers Phase 13 task 8 site
// B: when a path request arrives for a destination whose known path lives on a
...
// the local-client use-marking fires, isolating this call site from site D.
func TestPathRequestForLocalClientDestinationMarksUsed(t *testing.T) {
	...
	// below, so the cached-path answer aborts on loop prevention and site D
	// does NOT fire — only the local-client use-marking (site B) should.
...
// TestAnnounceMatchingPendingPathRequestMarksUsed covers Phase 13 task 8 site C:
...
// for site C: an announce that installs a path for a destination with NO
// outstanding path request must NOT mark the destination used (only the pending
// path-request membership gates the use-marking).
...
// TestPathRequestAnsweredFromCachedPathMarksUsed covers Phase 13 task 8 site D:
...
// answer branch). The cached path's interface is a non-local interface so site
// B does not fire, isolating this call site.
func TestPathRequestAnsweredFromCachedPathMarksUsed(t *testing.T) {
	...
	// Install a cached path on a NON-local interface so site B does not fire;
	// NextHop differs from any requestor transport id so the answer proceeds.
	announce := mustTestAnnouncePacketWithEmission(t, ts, id, dest, 1)
	ts.mu.Lock()
	ts.pathTable[string(dest.Hash)] = &PathEntry{
		Hops:       2,
